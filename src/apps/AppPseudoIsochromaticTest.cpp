...
)
{
    ...
    // Unload previous RYGB texture if any
    if (_currentRYGBTextureHandle != 0) {
        ctx.apis.UnloadRYGBTexture(_currentRYGBTextureHandle);
        _currentRYGBTextureHandle = 0;
    }
    ...
    if (!trial.rygb_path.empty()) {
        ...
        // Update texture handles for ImGui
        _subject.prompt.currentLandoltCTexture[ColorSpace::RGB] = rgbTex;
        _subject.prompt.currentLandoltCTexture[ColorSpace::OCV] = ocvTex;
    } else {
        // Fallback to old architecture (RGB/OCV PNG files)
        ...
        _subject.prompt.currentLandoltCTexture[ColorSpace::RGB] = ctx.apis.InitImGuiTexture(
            _subject.prompt.currentLandoltCTextureHandle[ColorSpace::RGB]
        );
        _subject.prompt.currentLandoltCTexture[ColorSpace::OCV] = ctx.apis.InitImGuiTexture(
            _subject.prompt.currentLandoltCTextureHandle[ColorSpace::OCV]
        );
    }
...
void AppPseudoIsochromaticTest::Init(TetriumApp::InitContext& ctx)
{
    ...
    // Note: RYGB transformation matrices are now loaded at engine level in Tetrium_Bootstrap.cpp
    // and set in TextureManager via SetRYGBTransformMatrices()
};
...
void AppPseudoIsochromaticTest::Cleanup(TetriumApp::CleanupContext& ctx)
{
    ...
    // Note: RYGB texture cleanup is handled in populatePromptContext when loading new trials
    // and will be cleaned up automatically when TextureManager is destroyed
